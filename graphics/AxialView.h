...
//##################################################################################################
class AxialView : public vpCanvas
{
Q_OBJECT

public:
    AxialView(QWidget *parent = 0);
    ~AxialView();

    /// Set the opengl texture ID for the volume to render.
    void SetVolumeTextureArray( uint handle );

    /// Set the opengl texture ID for the segmentation array to render.
    void SetSegmentTextureArray( uint handle );

    /// Set the opengl texture ID for the segmentation 3D mask to render.
    void SetSegmentTexture(uint handle);
    ...
    MacroSetMember(int, m_current_slice, CurrentSlice)
...
private:
    ...
    //###################################
    static const GLfloat cSlice[18];
    static const GLfloat cTexCoord[18];
    ...
    int         m_current_slice;    /**< current slice of the volume being displaced as an image */
    ...
    GLfloat     m_quad_vertices[18],   /**< vertex points for slice plane  */
                m_quad_colors[18];     /**< vertex points for slice colors */

    GLuint      m_program_slice;
    QString     m_path_to_slice_program[2] = {"../../FeatureVis/fvis/graphics/slice.vert.glsl","../../FeatureVis/fvis/graphics/slice.frag.glsl"};

    GLuint      m_vao,
                m_vbo_verts,           /**< vertex coordinates for slice-plane. */
    ...
    Texh        m_segment_array;      /**< Segmentation texture handler. Set from outside. */
