...
//#################################################################################################
float ComputeGradient2( const float stepSize, const vec3 lightDir, const vec3 scaled_pos,
                       const float ct_val, const sampler3D myTex )
{
    vec3 normLightDir = normalize(lightDir);
    vec3 sampleStep = stepSize * normLightDir;
    float ct_fx = texture( myTex, (scaled_pos + sampleStep) ).r;

    float lightGradient = 0.1f*((ct_fx-ct_val)/stepSize);
...
void main(void)
{
    ...
    for(int i = 0; i < totalSteps; i++)
    {
        ...
        if (label != 0)
        {
            ...
            if (range[0] == range[1])
                ct_color = texture(opticalTexture, vec2(intensity, label));
            else
            {
                //float scalar = clamp(intensity*maxValue, float(range[0]), float(range[1]));
            ...
            // If surface rendering is enabled for this segment:
            if (renderModes[label] == RENMODE_SURFACE)
            {
                //ct_color.a = float(optical_value.a)/255.0f;
                if (label == prev_label)
                    ct_color = vec4(0, 0, 0, 0);
            }
            else // use flat color and opacity for rendering the segment.
            //    ct_color.a = ct_color.a*intensity;
                ct_color.a *= 0.2f;
            ...
            if (ct_color.a > 0.001f)
            {
                ...
                //float grad_mag = length(gradient)/10.0f;
                //grad_mag = clamp( grad_mag, 0.0, 2.0 );
                //grad_mag = grad_mag < 0.1f ? 0 : grad_mag;
                //ct_color = clamp( ct_color * grad_mag, 0.0, 1.0 );
                ...
                //if( visibleFlags[VISFLAG_LIGHTING] == 0 )
